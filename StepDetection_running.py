...
class Worker(object):
    ...
    def Init(self):
        ...
        LP_Cutoff = 2.0 # low pass filter
        self.LP_b, self.LP_a = signal.butter(2, 2.0 * LP_Cutoff * self.delta_time, 'low') # to normalise extra 2 has been multipied w=Fc/(Fs/2)
    ...
    def Run(self):
        ...
        in_file = open('SensorData/UpperFloor_straightPath_12steps.csv', 'r')
        in_file.readline()
...
####### StepDisplacement ###############################
    ...
    def StepDetection(self):
        ...
        for i in range(data_length):
            acc_norm.append(
                math.sqrt(
                    data_process[i * 7 + 3] ** 2 + data_process[i * 7 + 4] ** 2 + data_process[i * 7 + 5] ** 2  # rootOf (Ax^2, Ay^2, Az^2)
                )
            )
        ...
        # plt.plot(data_process[i * 7 + 3], label='Raw acc X')
        # plt.plot(data_process[i * 7 + 4], label='Raw acc Y')
        # plt.plot(data_process[i * 7 + 5], label='Raw acc Z')
        ...
        # plt.plot(acc_norm) #####added
        # plt.xlabel('Time')
        # # naming the y axis
        # plt.ylabel('Acceleration')
        # plt.show()  #####added
        #
        # plt.plot(acc_LP)  #####added
        # plt.xlabel('Time')
        # # naming the y axis
        # plt.ylabel('Acceleration')
        # plt.show()  #####added
        ...
        for i in range(1, data_length - self.reserve_data_length):    # check 1 to 7-200
